...
anim_delta = delta_animation(data, lineplot, i_range = np.arange(0, 100), interval=10)

def save_anim(anim, fname, fps=60, **kwargs):
    writermp4 = animation.FFMpegWriter(fps=fps)
    anim.save(fname, writer=writermp4, **kwargs)

save_anim(anim_delta, f"{anim_dir}/delta_ylinear_{gauge}{name}.mp4", fps=10, dpi=200)
#plt.show()
